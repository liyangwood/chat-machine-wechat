...
(function(){
    ...
    var requestSelf = KG.request;

    var npmR = Meteor.npmRequire('request');
    ...
    var F = {
        reset : function(){
            groupList = friendList = curentUser = {};
        },
        ...
        doFriendList : function(item){
            ...
            if(flag < 25){
                ...
                //wx.getHeadImage(item, function(path){
                //    item.headUrl = path;
                //});
            }
        },
    ...
    var wx = {
        ...
        saveImage : function(name, buffer, callback){
        ...
        saveHeadImage : function(name, buffer, callback){
            var newFile = new FS.File();
            newFile.attachData(buffer, {type: 'image/png'}, (function(error){
                console.log(buffer);
                if(error) throw error;
        ...
        getMessageImage : function(id, callback){
            ...
            npmRequest(F.setOption({
                ...
                wx.saveImage(id, body, function(e, file){
                    callback('/getweixinimage?id='+file.name);

                });
            });
        },

        getHeadImage : function(item, callback){
            if(!item.UserName) return;

            var url = wx.config.root + item.HeadImgUrl;
            console.log(url);
            ...
            npmRequest(F.setOption({
                ...
                wx.saveHeadImage(item.UserName, body, function(e, file){
                    callback(file.name);
                });
        ...
        getTestImage : function(id, callback){
            ...
            //var url = wx.config.host + item.HeadImgUrl;
            //
            ////注意这里使用npmRequest是因为Meteor.http没有提供encoding参数的实现
            //
            //npmRequest(F.setOption({
